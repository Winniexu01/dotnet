...
public class LicenseScanTests : TestBase
{
    ...
    private static readonly string[] s_allowedLicenseExpressions = new string[]
    {
        ...
        "nist-software", // https://github.com/nexB/scancode-toolkit/blob/develop/src/licensedcode/data/licenses/nist-software.LICENSE
    ...
    public void ScanForLicenses()
    {
        ...
        ExecuteHelper.ExecuteProcessValidateExitCode(
            "scancode",
            $"--license --processes 4 --max-in-memory 100000000 --timeout {FileScanTimeoutSeconds} --strip-root --only-findings {ignoreOptions} --json-pp {scancodeResultsPath} {Config.LicenseScanPath}",
            OutputHelper);
